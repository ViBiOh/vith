...
func (a App) generateStream(ctx context.Context, req model.Request) error {
	...
	cmd := exec.CommandContext(ctx, "ffmpeg", "-i", inputName, "-codec:v", "libx265", "-preset", "superfast", "-codec:a", "aac", "-b:a", "128k", "-ac", "2", "-y", "-f", "hls", "-hls_time", "4", "-hls_playlist_type", "event", "-hls_flags", "independent_segments", "-threads", "2", outputName)
